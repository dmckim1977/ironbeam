...
class SecurityDefinition(BaseModel):
    ...
    def get_activation_time(self) -> Optional[datetime]:
        ...
        return datetime.fromtimestamp(
            self.activation_time / 1000) if self.activation_time else None

    def get_expiration_time(self) -> Optional[datetime]:
        """Convert expiration_time to datetime."""
        return datetime.fromtimestamp(
            self.expiration_time / 1000) if self.expiration_time else None

    def get_creation_date(self) -> Optional[datetime]:
        """Convert creation_date to datetime."""
        return datetime.fromtimestamp(
            self.creation_date / 1000) if self.creation_date else None
...
class SecurityMarginAndValue(BaseModel):
    ...
    initial_margin_short: Optional[float] = Field(None,
                                                  alias='initialMaginShort')  # Note API typo in 'Magin'
    ...
    margin_schedule_details: Optional[List[MarginScheduleDetail]] = Field(None,
                                                                          alias='marginScheduleDetails')

    def get_current_time(self) -> Optional[datetime]:
        """Convert current_time to datetime."""
        return datetime.fromtimestamp(
            self.current_time / 1000) if self.current_time else None
